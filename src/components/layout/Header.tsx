...
export default function Header() {
  ...
  return (
    <>
      <header className={`h-16 border-b flex items-center justify-between px-4 lg:px-6 sticky top-0 z-20 ${isDark ? 'bg-[#1E293B] border-slate-700' : 'bg-white border-slate-200'}`}>
      ...
      {mobileMenu && (
        <div className="lg:hidden fixed inset-0 z-30 bg-black/50" onClick={() => setMobileMenu(false)}>
          <div className={`w-64 h-full p-4 shadow-xl ${isDark ? 'bg-[#1E293B] border-l border-slate-700' : 'bg-white border-l border-slate-200'}`} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-6 px-2">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDark ? 'bg-primary-600' : 'bg-primary-500'}`}>
                <HeartPulse className="w-6 h-6 text-white" />
              </div>
              <div>
                <h2 className={`font-bold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>VitaNexo</h2>
                <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{user?.first_name} {user?.last_name}</p>
              </div>
            </div>
