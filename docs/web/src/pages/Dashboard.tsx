...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto px-2 md:px-4">
      <div className="flex flex-col gap-2 mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-purple-100">Bienvenido, Roberto 👋</h1>
        <p className="text-slate-500 dark:text-slate-400">Panel principal · {new Date().toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
      </div>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="value" stroke="#8B5CF6" strokeWidth={2} fill="url(#glucoseGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
